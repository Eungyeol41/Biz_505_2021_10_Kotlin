...
fun main() {
    ...
    var str = "두 수 ${num1}과 $num2 의 덧셈 결과 : ${num1 + num2}"
    println(str)
    ...
    // var str1 = null : (X) Kotlin, 초기값으로 null 불가능!
 ...
 *
 * 단지 변수 type을 지정하지 않을 뿐 한 번 선언된 변수에 다른 type의 데이터(값)을 저장할 수는 없다.
 *
 * 변수를 선언할 때 변수들의 초기값은 null이 될 수 없다.
 *
 * Kotlin에서도 변수형을 지정하여 선언할 수 있다.
 * Kotlin의 변수 type은 class type이 없다
 *  모두 primitive 방식이다.
 * 단, 모든 변수 type의 키워드는 첫 글자가 대문자이다.
 *
 * 변수 type을 지정하지 않아도 선언을 할 수 있지만 굳이 type을 지정하는 이유는
 *  Kotlin 문법 중에 변수의 초기화를 미루기(유예하기)라는 기능 때문이다.
 */
